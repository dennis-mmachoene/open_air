...
export const screens: Specimen[] = [
      ...
      <div className="overflow-hidden rounded-xl border border-p-border">
        <div className="flex items-center justify-between border-b border-p-border px-4 py-2 text-sm">
          ...
          <div className="flex items-center gap-3 text-p-text-soft">
            ...
            <span className="rounded-full bg-p-primary px-3 py-1 text-xs font-medium text-p-on-primary">Start free</span>
          </div>
        </div>
        <div className="flex flex-col items-center gap-3 px-6 py-8 text-center" style={{ backgroundColor: "var(--p-bg)" }}>
          <span className="rounded-full bg-p-surface-2 px-3 py-1 text-xs text-p-text-soft">New · v2 is here</span>
          ...
          <div className="flex gap-2">
            <button className="rounded-lg bg-p-primary px-4 py-2 text-sm font-medium text-p-on-primary">Get started</button>
            <button className="rounded-lg border border-p-border px-4 py-2 text-sm text-p-text">Live demo</button>
          </div>
          <div className="mt-3 grid grid-cols-3 gap-3">
            {[1, 2, 3].map((n) => (
              <div key={n} className="rounded-lg border border-p-border p-3 text-left">
    ...
    render: () => (
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={name as string} className="flex flex-col gap-2 rounded-xl border p-4" style={featured ? { borderColor: "var(--p-primary)", backgroundColor: "var(--p-surface)" } : { borderColor: "var(--p-border)" }}>
            ...
            <button className="mt-2 rounded-lg px-3 py-1.5 text-sm font-medium" style={featured ? { backgroundColor: "var(--p-primary)", color: "var(--p-on-primary)" } : { border: "1px solid var(--p-border)", color: "var(--p-text)" }}>Choose</button>
          </div>
        ))}
      </div>
    ),
  },
  {
    ...
    title: "Sign in",
    render: () => (
      <div className="mx-auto flex max-w-xs flex-col gap-3 rounded-xl border border-p-border p-4">
        <h3 className="font-display text-lg text-p-text">Welcome back</h3>
        <input placeholder="Email" className="rounded-lg border border-p-border bg-p-bg px-3 py-2 text-sm text-p-text outline-none" />
        <input placeholder="Password" type="password" className="rounded-lg border border-p-border bg-p-bg px-3 py-2 text-sm text-p-text outline-none" />
        <button className="rounded-lg bg-p-primary px-3 py-2 text-sm font-medium text-p-on-primary">Sign in</button>
        <button className="rounded-lg border border-p-border px-3 py-2 text-sm text-p-text">Continue with Google</button>
      </div>
    ),
  },
  {
    ...
    span: 3,
    render: () => (
      <div className="flex gap-3 rounded-xl border border-p-border p-3" style={{ backgroundColor: "var(--p-bg)" }}>
        <div className="hidden w-32 shrink-0 flex-col gap-1 sm:flex">
          {["Overview", "Reports", "Customers", "Settings"].map((l, i) => (
            <span key={l} className="rounded-md px-2 py-1.5 text-sm" style={i === 0 ? { backgroundColor: "var(--p-surface-2)", color: "var(--p-text)" } : { color: "var(--p-text-soft)" }}>{l}</span>
          ))}
        </div>
        <div className="flex flex-1 flex-col gap-3">
          <div className="grid grid-cols-3 gap-3">
            {[["MRR", "$48k"], ["Users", "1.2k"], ["NPS", "62"]].map(([l, v]) => (
              <div key={l} className="rounded-lg border border-p-border bg-p-surface p-2">
                <p className="text-xs text-p-text-muted">{l}</p>
                <p className="text-lg font-semibold text-p-text">{v}</p>
              </div>
            ))}
          </div>
          <div className="rounded-lg border border-p-border bg-p-surface p-3">
    ...
    render: () => (
      <div className="mx-auto w-40 overflow-hidden rounded-[1.6rem] border-4 p-2" style={{ borderColor: "var(--p-text)", backgroundColor: "var(--p-bg)" }}>
        ...
        <div className="flex flex-col gap-2">
          <div className="h-14 rounded-lg" style={{ background: "linear-gradient(135deg, var(--p-c1), var(--p-c4))" }} />
          {[1, 2].map((n) => (
            <div key={n} className="flex items-center gap-2 rounded-lg border border-p-border p-1.5">
    ...
    render: () => (
      <div className="grid grid-cols-3 gap-3">
        {[["To do", 2, "c1"], ["Doing", 1, "c3"], ["Done", 2, "c5"]].map(([col, n, c]) => (
          <div key={col as string} className="flex flex-col gap-2 rounded-lg bg-p-surface-2 p-2">
            <p className="flex items-center gap-1.5 text-xs font-medium text-p-text-soft">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: `var(--p-${c})` }} />{col}
            </p>
            {Array.from({ length: n as number }).map((_, i) => (
              <div key={i} className="rounded-md border border-p-border bg-p-surface p-2">
