    ...
    <div className="flex flex-col gap-8">
      <label className="flex flex-col gap-1 text-sm text-text-soft">
        ...
        <div className="flex items-center gap-2">
          <input type="color" value={normalized ?? "#4f46e5"} onChange={(e) => setHex(e.target.value)} aria-label="Brand color" className="h-10 w-12 cursor-pointer rounded-lg border border-border bg-surface p-1" />
          <input value={hex} onChange={(e) => setHex(e.target.value)} spellCheck={false} className={clsx("w-32 rounded-lg border bg-surface px-3 py-2 font-mono text-sm text-text outline-none", valid ? "border-border focus-visible:border-text" : "border-p-danger")} />
        </div>
      </label>

      {levels ? (
        <>
          <div className="grid grid-cols-2 gap-6 rounded-2xl border border-border bg-surface-2 p-8 sm:grid-cols-3 lg:grid-cols-5">
            {levels.map((l) => (
              <div key={l.level} className="flex flex-col items-center gap-3">
                <div className="flex h-20 w-full items-center justify-center rounded-xl bg-canvas text-sm font-medium text-text" style={{ boxShadow: l.boxShadow }}>
                  {l.level}
                </div>
