    ...
    <div className="flex h-64 overflow-hidden rounded-2xl border border-border sm:h-80">
      {swatches.map((s) => (
        <button
          ...
          title={`Copy ${s.hex}`}
        >
          <span className="absolute inset-x-0 bottom-0 flex flex-col items-center gap-0.5 p-3 opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">
