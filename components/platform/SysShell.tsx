        ...
        <aside className="shrink-0 border-b border-border md:min-h-screen md:w-60 md:border-b-0 md:border-r">
          <div className="flex flex-col gap-1 p-4">
            <div className="mb-3 flex items-center gap-2">
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-text text-xs font-bold text-canvas">SA</span>
              ...
              {NAV.map((n) => {
                ...
                return (
                  <Link
                    ...
                    className={
                      on
                        ? "rounded-lg bg-surface-2 px-3 py-2 text-sm font-medium text-text"
                        : "rounded-lg px-3 py-2 text-sm text-text-soft transition-colors hover:bg-surface-2 hover:text-text"
                    }
                  >
                    {n.label}
                  </Link>
        ...
        <main className="min-w-0 flex-1">
          <header className="flex items-center justify-between gap-3 border-b border-border px-5 py-3">
            <p className="text-xs text-text-muted">
              ...
              <span className="ml-2 rounded-full border border-border px-2 py-0.5 capitalize">{admin.role.replace("_", " ")}</span>
            </p>
            <div className="flex items-center gap-3">
              ...
              <form action={logoutAction}>
                <button className="rounded-full border border-border px-3 py-1.5 text-xs font-medium text-text transition-colors hover:bg-surface-2">Sign out</button>
              </form>
            </div>
          </header>
