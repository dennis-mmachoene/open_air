        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          {canManage ? (
            <form action={deleteKitAction}>
              ...
              <button className="rounded-full border border-p-danger/40 px-4 py-2 text-sm font-medium text-p-danger hover:bg-p-danger/5">Delete kit</button>
            </form>
          ) : null}
        </div>
      </header>
      ...
      {canManage && pending.length > 0 ? (
        <section className="flex flex-col gap-3 rounded-2xl border border-amber-600/40 bg-amber-600/5 p-5">
          ...
          <ul className="flex flex-col gap-3">
            {pending.map((p) => (
              <li key={p.id} className="flex flex-col gap-2 rounded-xl border border-border bg-canvas p-3">
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  <span className="rounded-full border border-border px-2 py-0.5 text-xs font-medium">{TYPE_LABEL[p.type]}</span>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <form action={approveProposalAction}>
                    ...
                    <button className="rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white hover:opacity-90">Approve</button>
                  </form>
                  <form action={rejectProposalAction} className="flex items-center gap-1">
                    <input type="hidden" name="slug" value={slug} /><input type="hidden" name="kitSlug" value={kit.slug} /><input type="hidden" name="id" value={p.id} />
                    <input name="note" placeholder="Reason (optional)" className="w-40 rounded-lg border border-border bg-surface px-2 py-1 text-xs text-text focus:border-text focus:outline-none" />
                    <button className="rounded-lg border border-border px-3 py-1.5 text-xs text-text-soft hover:bg-surface-2">Reject</button>
                  </form>
                </div>
              </li>
            ))}
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {kit.assets.map((a) => (
            <li key={a.id} className="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-4">
              ...
              {a.type === "palette" ? <Strata hexes={a.hexes} className="h-14" /> : <div className="h-14 rounded-xl border border-border" style={{ background: a.hexes[0] }} />}
      ...
      {canManage ? (
        <form action={addAssetAction} className="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-5">
          ...
          <div className="flex flex-wrap gap-2">
            <input name="name" placeholder="Name (e.g. Primary, Ramp)" required className="min-w-0 flex-1 rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-text focus:border-text focus:outline-none" />
            <input name="hexes" placeholder="#1d4ed8 or #1d4ed8, #3b82f6, …" required className="min-w-0 flex-[2] rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-text focus:border-text focus:outline-none" />
          </div>
          <input name="notes" placeholder="Notes (optional)" className="rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-text focus:border-text focus:outline-none" />
          <button className="w-fit rounded-full bg-text px-5 py-2 text-sm font-medium text-canvas hover:opacity-90">Add</button>
          <p className="text-xs text-text-muted">One hex = a color swatch; multiple = a palette.</p>
        </form>
      ) : (
        <form action={proposeAddAction} className="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-5">
          <input type="hidden" name="slug" value={slug} /><input type="hidden" name="kitSlug" value={kit.slug} /><input type="hidden" name="kitId" value={kit.id} />
          ...
          <div className="flex flex-wrap gap-2">
            <input name="name" placeholder="Name" required className="min-w-0 flex-1 rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-text focus:border-text focus:outline-none" />
            <input name="hexes" placeholder="#1d4ed8 or #1d4ed8, #3b82f6, …" required className="min-w-0 flex-[2] rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-text focus:border-text focus:outline-none" />
          </div>
          <input name="note" placeholder="Why this change? (optional)" className="rounded-xl border border-border bg-canvas px-3 py-2 text-sm text-text focus:border-text focus:outline-none" />
          <button className="w-fit rounded-full bg-text px-5 py-2 text-sm font-medium text-canvas hover:opacity-90">Submit proposal</button>
        </form>
      )}
    </div>
  );
