        ...
        <SectionCard title="Two-factor authentication">
          {row?.totpEnabled ? (
            <div className="flex flex-col gap-3">
              <p className="inline-flex w-fit items-center gap-2 rounded-full border border-green-600/40 px-3 py-1 text-sm text-green-700 dark:text-green-400">
                <span className="h-2 w-2 rounded-full bg-green-600" /> Enabled
              </p>
              <p className="text-sm text-text-soft">{remaining} backup code{remaining === 1 ? "" : "s"} remaining.</p>
              {backupCodes.length > 0 ? (
                <div className="rounded-xl border border-amber-600/40 bg-amber-600/5 p-4">
              ...
              <form action={disableTotpAction}>
                <button className="w-fit rounded-full border border-p-danger/40 px-4 py-2 text-sm font-medium text-p-danger hover:bg-p-danger/5">Disable 2FA</button>
              </form>
            </div>
          ) : pendingSecret ? (
            <div className="flex flex-col gap-3">
              ...
              <form action={confirmTotpAction} className="flex flex-wrap items-center gap-2">
                <input name="code" inputMode="numeric" placeholder="6-digit code" required className="w-40 rounded-xl border border-border bg-canvas px-3 py-2 text-center tracking-widest text-text focus:border-text focus:outline-none" />
                <button className="rounded-full bg-text px-4 py-2 text-sm font-medium text-canvas hover:opacity-90">Confirm & enable</button>
              </form>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              ...
              <form action={startTotpAction}>
                <button className="w-fit rounded-full bg-text px-4 py-2 text-sm font-medium text-canvas hover:opacity-90">Set up 2FA</button>
              </form>
            </div>
          )}
        </SectionCard>
        ...
        <SectionCard
          ...
          action={
            <form action={revokeAllOtherSessionsAction}>
              <button className="rounded-full border border-p-danger/40 px-3 py-1.5 text-xs font-medium text-p-danger hover:bg-p-danger/5">Revoke all & sign out</button>
            </form>
          }
        >
            ...
            {sessions.map((s) => (
              <li key={s.id} className="flex flex-wrap items-center gap-3 border-b border-border py-3 last:border-0">
                ...
                <form action={revokeSessionAction}>
                  ...
                  <button className="rounded-lg border border-border px-2.5 py-1 text-xs text-text-soft hover:bg-surface-2">Revoke</button>
                </form>
              </li>
            ))}
          </ul>
